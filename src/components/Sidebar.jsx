...
export default function Sidebar({ vistaActual, setVistaActual, usuario, socketConectado, onLogout }) {
  ...
  return (
    <aside className={`bg-slate-950 border-r border-slate-800 flex flex-col justify-between shrink-0 select-none transition-all duration-200 relative h-full ${colapsado ? 'w-20' : 'w-64'}`}>
      <button onClick={() => setColapsadoManual(!colapsadoManual)} className="hidden lg:flex absolute -right-3 top-7 z-30 w-6 h-6 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full items-center justify-center text-xs shadow-lg cursor-pointer border border-slate-800 active:scale-95">
        {colapsado ? '▶' : '◀'}
      </button>

      <div className="p-4 md:p-6 border-b border-slate-900 flex items-center gap-3 shrink-0 overflow-hidden">
        <div className="w-10 h-10 bg-gradient-to-tr from-indigo-600 to-violet-500 rounded-xl flex items-center justify-center font-black text-white shadow-md shrink-0 text-lg">S</div>
        {!colapsado && (
          ...
          <div key={grupo.seccion} className="space-y-1">
            {!colapsado && (
              <div className="px-3.5 pb-1 flex items-center gap-2">
                <span className="text-[9px] font-black uppercase tracking-widest text-indigo-400/70">{grupo.seccion}</span>
